...
def genetic_algorithm(employees: list[Employee], missions: list[Mission], centers: list[Center], distance_matrix: list[list[float]], size: int, crossover_rate: float, mutation_rate: float, max_execution_time: int, k: int) -> Solution:
	...
	while time() - start_time < max_execution_time:
		population = genetic_algorithm_iteration(employees, missions, population, distance_matrix, size, crossover_rate, mutation_rate, k)

	return pick_best_solutions(population, employees, missions, distance_matrix, 1)[0]


def genetic_algorithm_iteration(employees: list[Employee], missions: list[Mission], population: list[Solution], distance_matrix: list[list[float]], size: int, crossover_rate: float, mutation_rate: float, k: int) -> list[Solution]:
	"""
